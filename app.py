...
def load_data_from_gsheets():
    try:
        ...
        # KHÔI PHỤC CẤU TRÚC ĐẦY ĐỦ CHO GSPREAD
        credentials = {
            "type": "service_account",
            "project_id": "streamlit-gdrive-connector",  # Tên giả, không bắt buộc
            "private_key_id": "key_id_placeholder",     # ID giả, không bắt buộc
            "private_key": st.secrets["gdrive"]["private_key"].replace('\\n', '\n'),
            "client_email": st.secrets["gdrive"]["client_email"],
            "client_id": "client_id_placeholder",       # ID giả, không bắt buộc
        ...
        for title in sheet_titles:
            try:
                ...
                st.error(f"❌ Không tìm thấy sheet '{title}' trong Google Sheet.")
        ...
        st.error(
            "❌ Lỗi kết nối hoặc cấu hình Google Sheet.\n"
            f"Chi tiết: {e}"
        )
        return None
